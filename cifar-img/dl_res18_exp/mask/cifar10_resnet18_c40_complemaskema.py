...
train_cfg = dict(type='resEpochTrainLoop', max_epochs=50, method_epoch=1, method_start=method_start, val_interval=5) #, val_interval=1
val_cfg = dict()
test_cfg = dict()
...
default_hooks = dict(
    ...
    # enable ema of masking target model. DO NOT USE CUSTOM FOR LIST ISSUE WITH CFG-OPTIONS
    emahook=dict(type='EMAHook', momentum=0.2, begin_epoch=method_start, priority='ABOVE_NORMAL', evaluate_on_origin=True),
)
# enable ema of masking target model.
# custom_hooks = dict(type='EMAHook', momentum=0.2, begin_epoch=method_start, priority='ABOVE_NORMAL', evaluate_on_origin=True)
