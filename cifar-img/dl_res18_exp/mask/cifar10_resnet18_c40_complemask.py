...
model = dict(
    ...
    train_cfg=dict(
        is_invold=True, # remember to turn this off for mask_inv = False?
        # cls_only=True,
    ),
    )
...
# set visualizer
vis_backends = dict(type='WandbVisBackend', init_kwargs=dict(project='msk_exp',id='followexpname',group="mskfix"))
# vis_backends = dict(type='WandbVisBackend', init_kwargs=dict(project='test_config',id='mskmoderatio_v',group="msk"))#msk_full
##################################################################### msk_exp
...
expname='KDmsk__step25_ep50_lwxxx'