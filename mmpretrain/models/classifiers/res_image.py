...
class resImageClassifier(BaseClassifier):
    ...
    def __init__(self,
        ...
        if train_cfg is not None:
            self.is_invold = train_cfg.pop('is_invold', False)
            if not self.is_invold:
                head.mask_inv = False # is this working?
    ...
    def method_init(self):
        ...
        if self.is_ema and not self.is_old:
            from mmengine.model import BaseAveragedModel, ExponentialMovingAverage, MomentumAnnealingEMA, StochasticWeightAverage
            def get_average(mode='ema'):
                if 'ema' in mode:
                    return ExponentialMovingAverage
                elif 'anne' in mode:
                    return MomentumAnnealingEMA
                elif 'sto' in mode:
                    return StochasticWeightAverage
                else:
                    return BaseAveragedModel
            average_model = get_average(self.ema_mode)
            self.clhead = average_model(self.head)
            if not self.cls_only:
                self.clbackbone = average_model(self.backbone)
                self.clneck = average_model(self.neck)          
        else:
    ...
    def foward_old(self, inputs):
        if self.cls_only:
            return tuple([self.clhead])
        x = self.clbackbone(inputs)
        x = self.clneck(x)
        # if self.is_invold:
        #     return tuple([x, self.clhead])
        x = self.clhead(x)
        return x
