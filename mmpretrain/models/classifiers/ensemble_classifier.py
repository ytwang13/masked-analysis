...
class ensImageClassifier(BaseClassifier):
    ...
    def __init__(self,
                 ...
                 ensemble_model: Optional[dict] = None,
                 ensemble_ckp_list: Optional[str] = None,
        ...
        super(ensImageClassifier, self).__init__(
        ...
        for ckp in ensemble_ckp_list:
            self.ensemble.append(self.freeze(init_model(ensemble_model, ckp, device=get_device())))
    ...
    def ensemble_forward(self, inputs):
        # output = [model(inputs).unsqueeze(0) for model in self.ensemble] #.softmax(dim=1)
        return torch.mean(torch.cat([ensemble(inputs).unsqueeze(0) for ensemble in self.ensemble],dim=0), dim=0)
    ...
    def loss(self, inputs: torch.Tensor,
        ...
        """
        feats = self.extract_feat(inputs)
        ensemble_feats = self.ensemble_forward(inputs).detach()#.softmax(dim=1)
        return self.head.loss(feats, data_samples, ensemble_feats)
