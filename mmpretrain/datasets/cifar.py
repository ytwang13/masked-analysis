...
class CIFAR10(BaseDataset):
    ...
    def load_data_list(self):
        ...
        # new_order = np.random.permutation(len(self.CLASSES))
        for img, gt_label in zip(imgs, gt_labels):
            # info = {'img': img, 'gt_label': int(new_order[gt_label])}
...
class CIFAR10TEST(BaseDataset):
    ...
    def load_data_list(self):
        ...
        downloaded_list = self.kaggle_list
        ...
        # load the picked numpy arrays
        file_path = join_path(root, self.base_folder, self.kaggle_list[-1])
        import mmengine
        entry = mmengine.load(file_path)
        imgs.append(entry[b'data'])
